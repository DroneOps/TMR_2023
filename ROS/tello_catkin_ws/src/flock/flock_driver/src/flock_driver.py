    ...
    def videoFrameHandler(self, event, sender, data):
        ...
        if len(data) != 1460:
            ...
            for frame in self._h264_decode(self.packet_data):
                ...
                cropped_frame = color_mat[0:(frame_height-cropped_pixels), 0:frame_width]
                smoothed_frame = cv2.GaussianBlur(cropped_frame,blur_kernel,0)
                #smoothed_frame = cv2.bilateralFilter(cropped_frame,9,75,75)
                # Convert OpenCV Mat => ROS Image message and publish
                img_msg = self._cv_bridge.cv2_to_imgmsg(smoothed_frame, 'bgr8')
