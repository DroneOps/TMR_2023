...
Node::Node (ORB_SLAM2::System::eSensor sensor, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) {
  ...
  node_handle_.param(name_of_node_ + "/use_viewer", use_viewer_param, true);
  ...
  isLost.data = false;
  lost_publisher_ = node_handle_.advertise<std_msgs::Bool>(name_of_node_+"/isLost", 1);
}
...
sensor_msgs::PointCloud2 Node::MapPointsToPointCloud (std::vector<ORB_SLAM2::MapPoint*> map_points) {
  ...
  if(orb_slam_->GetTrackingState() != 2){
    isLost.data = true;
    lost_publisher_.publish(isLost);
    std::cout << orb_slam_->GetTrackingState() << std::endl;
  }else{
    isLost.data = false;
    lost_publisher_.publish(isLost);
  }
